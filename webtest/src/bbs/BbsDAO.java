...
public class BbsDAO {
	...
	public List<BbsDTO> list(Map map) {
		...
		sql.append("         wdate, grpno, indent, ansnum, r  ");
		sql.append("     from(  ");
		sql.append("         SELECT bbsno, wname, title, viewcnt,   ");
	...
	public boolean create(BbsDTO dto) {
		...
		sql.append(" values((select nvl(Max(bbsno),0)+1 as bbsno ");
		sql.append(" from bbs), ");
		sql.append(" ?, ?, ?, ?, sysdate,  ");
	...
	public boolean createReply(BbsDTO dto) {
		...
		try {
			...
			pstmt.setString(3, dto.getContent());
			pstmt.setString(4, dto.getPasswd());
			pstmt.setInt(5, dto.getGrpno());// 부모와 같아야하고
			pstmt.setInt(6, dto.getIndent() + 1);// 부모보다+1증가해야함
	...
	public BbsDTO read(int bbsno) {
		...
		ResultSet rs = null;
		StringBuffer sql = new StringBuffer();
		sql.append(" select bbsno, wname, title, content, viewcnt, to_char(wdate,'yyyy-mm-dd')  wdate");
		sql.append(" from bbs ");
		sql.append(" where bbsno = ?");
